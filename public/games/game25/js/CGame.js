function CGame(oData){
    ...
    var _aStartingPlayerHighHand;
    var _aStartingPlayerLowHand;
    ...
    this.changeState = function(iState){
        ...
        switch(iState){
            case STATE_GAME_WAITING_FOR_BET:{
                    _oInterface.displayMsg(TEXT_DISPLAY_MSG_WAITING_BET,TEXT_MIN_BET+": "+formatValue(MIN_BET) + "\n" + TEXT_MAX_BET+": "+formatValue(MAX_BET));
                    break;
            }
    ...
    this._playerWin = function(iTotWin){
        ...
        _oSeat.increaseCredit(iTotWin,'1');
        _iGameCash -= iTotWin;
        _oInterface.displayMsg(TEXT_DISPLAY_MSG_SHOWDOWN,TEXT_DISPLAY_MSG_PLAYER_WIN + " " + formatValue(iTotWin));
    ...
    this.setBet = function(iFicheIndex){
        ...
        if( _oSeat.getBetAnte() + iFicheValue > _iMaxBet){
       ...
       _oCardContainer.removeAllChildren();
        ...
        if(iRandWin > WIN_OCCURRENCE){
            ...
            do{
                ...
                console.log("_iPlayerLowValue "+_iPlayerLowValue)
                console.log("_iPlayerHighValue "+_iPlayerHighValue)
                console.log("_iPairValueLowHand "+_iPairValueLowHand)
                console.log("_iPairValueHighHand "+_iPairValueHighHand)
                console.log("_aPlayerLowHand"+JSON.stringify(_aPlayerLowHand))
                console.log("_aPlayerHighHand"+JSON.stringify(_aPlayerHighHand))
            }while((_szHandResult === "player" && _bPlayerWinsLowHand) || !this._checkIfHighHandBetterThanLow());
    ...
    this._compareLowHands = function(){
        ...
        if(_aPlayerLowHand[0].rank === _aPlayerLowHand[1].rank || _aPlayerLowHand[0].rank === CARD_JOKER || _aPlayerLowHand[1].rank === CARD_JOKER){
            _iPlayerLowValue = ONE_PAIR;
            _iPairValueLowHand = _aPlayerLowHand[0].rank;
            if(_aDealerLowHand[0].rank === _aDealerLowHand[1].rank || _aDealerLowHand[0].rank === CARD_JOKER || _aDealerLowHand[1].rank === CARD_JOKER){
    ...
    this.onSplitHand = function(){
        ...
        _aStartingPlayerHighHand = _aPlayerHighHand.slice();
        _aStartingPlayerLowHand = _aPlayerLowHand.slice();
       ...
       */
        if(this._checkIfHighHandBetterThanLow()){
            ...
            _aPlayerHighHand = _aStartingPlayerHighHand.slice();
            _aPlayerLowHand = _aStartingPlayerLowHand.slice();
        }
