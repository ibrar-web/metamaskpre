...
BasicGame.Game.prototype = {
	...
	CheckWinlines: function (CheckLine) {
		...
		if (match > 0) {
			...
			if (this.number_format(win) != 0) {
				...
				this.c_win += 0;
				CREDITS += 0;
				$.post("update6", { _token: $('meta[name=csrf-token]').attr('content'), amount: CREDITS + win }, function (t) {
	...
	updateCREDITS: function () {
		//console.log(CREDITS);
		$.post("update6", { _token: $('meta[name=csrf-token]').attr('content'), amount: CREDITS }, function (t) {
			$.jStorage.set(GAME_NAME + "_user_credits", t);
		});
